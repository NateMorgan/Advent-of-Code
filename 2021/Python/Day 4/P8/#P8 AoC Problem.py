...
def bingo_winner(Boards,Selections):
    ...
    for num in Selections:
        for board in range(len(marked_board)):
            ...
            if (True in row_check or True in column_check) and board not in winning_order:
                winning_order.append(board)
                if len(winning_order) == len(marked_board):
                    sum = 0
...
assert main("D4 Test.txt") == 1924, "Function does not match example given"
