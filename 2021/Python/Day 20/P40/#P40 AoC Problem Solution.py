    ...
    for enhan in range(Enhances):
        ...
        for row in range(len(binary_input)):
            for col in range(len(binary_input[row])):
                ...
                binary_output[row][col] = enhance_algo_binary[int(enhance_num,2)]
        binary_input = [[i for i in row] for row in binary_output]
        image_output = [[rev_binary_decode[i] for i in row] for row in binary_output]

    lit_count = sum(sum(x) for x in binary_output)

    return lit_count, image_output

assert main("D20 Test.txt",50)[0] == 3351, "Function does not match example given"

if __name__ == "__main__":
    print(main("D20 Input.txt",50)[0])